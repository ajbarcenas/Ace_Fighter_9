...
int authScores(){
    ...
    BIO *outbio = NULL;
    //-------------------------------------------------------
    //Fixed error: invalid conversion from 'const SSL_METHOD* 
	...
	char pagename[256] = "odin.cs.csub.edu";
	char file[256] = "/~abarcenas/3350/Game/HighScores.php"
    int port = PORT;
    ...
    if(SSL_library_init() < 0) BIO_printf(outbio, "Could
            not initialize the OpenSSL library !\n");
    ...
    if (connect(sd, (struct sockaddr*)&addr, sizeof(addr)) == -1) {
        BIO_printf(outbio, "%s: Cannot connect to host %s [%s] on port
	...
	//--------------------------------------------
    //Fixed warning: 'if' clause does not gaurd... [-Wmisleading-indentation]
    //Previous code was:
    //  if (ret <= 0) 
