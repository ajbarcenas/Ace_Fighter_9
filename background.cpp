...
void init_opengl(void)
{
    ...
    glGenTextures(1, &g.playerSilhouetteTexture);
    ...
    //-------------------------------------------------------------------------
    //playermodel layer by Andrew Oliveros
    
    int wpl = img[8].width;
    int hpl = img[8].height;

    glBindTexture(GL_TEXTURE_2D, g.playerTexture);

    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, 3, wpl, hpl, 0,
            GL_RGB, GL_UNSIGNED_BYTE, img[8].data);

    //Silhouette

    glBindTexture(GL_TEXTURE_2D, g.playerSilhouetteTexture);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);

    unsigned char *playerSilhouetteData = buildAlphaData(&img[8]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, wpl, hpl, 0,
            GL_RGBA, GL_UNSIGNED_BYTE, playerSilhouetteData);
    free(playerSilhouetteData);
...
void render()
{
   ...
   //-------------------------------------------------------------------------
    ...
    glPushMatrix();
    float w = (g.xres/20)-18;
    float h = (g.yres/20)-18;
    ...
    //PlayerModel
    //Done by Andrew Oliveros
    //glClear(GL_COLOR_BUFFER_BIT);
    //glColor3f(1.0, 1.0, 1.0);
    //glColor4ub(255, 255, 255, 255);
    glTranslatef(p->center.x, p->center.y, p->center.z);
    glBegin(GL_QUADS);
    glTexCoord2f(g.tex.xc[8], g.tex.yc[9]); glVertex2i(-w, -h);
    glTexCoord2f(g.tex.xc[8], g.tex.yc[8]); glVertex2i(-w, h);
    glTexCoord2f(g.tex.xc[9], g.tex.yc[8]); glVertex2i(w, h);
    glTexCoord2f(g.tex.xc[9], g.tex.yc[9]); glVertex2i(w, -h);
    glEnd();
    glPopMatrix();
    glDisable(GL_ALPHA_TEST);
    glBindTexture(GL_TEXTURE_2D, 0);
